...
class ABCTelegramBotError(ABCError): pass
class ABCTelegramDispatcher(ABCError): pass
...
class ABCTelegram(ABClass):
	...
	def polling(self):
		process = Process(target=self.executor.start_polling, args=(self.dp,), kwargs={'skip_updates': True, 'loop': self.loop})
		process.start()